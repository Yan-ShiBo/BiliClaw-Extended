...
发现结果经过多维度多样性选择：平台族预留配额（默认保存 B 站 / 小红书 / 抖音 / YouTube = 8 / 1 / 1 / 1，可通过 `[scheduler.pool_source_shares]` 配置；默认有效配比只有 B 站，小红书 / 抖音 / YouTube 需显式开启）→ 主题去重 → 风格均衡 → **跨平台混排** → raw ceiling 封顶。B 站四策略统一计入 `bilibili`，小红书统一计入 `xiaohongshu`，抖音 search / hot / feed 统一计入 `douyin`，YouTube `yt_search` / `yt_trending` / `yt_channel` 统一计入 `youtube`；被关闭的平台会从有效配比中剔除。`pool_target_count` 表示前端真实可换目标，raw 素材可在 `max(target*2, target+120)` 的成本上限内承接 topic window / 预生成 / XHS token 折损。

前端展示的候选池数字以 runtime status 为准：`pool_available_count` 只表示已经有推荐文案、分类、可跳转链接且未进入近期已看窗口的可换内容；`pool_pending_count` 表示已经发现但还在整理的素材。后台补池按 available-by-source 计算缺口，并用 raw-material headroom 防止无效 churn；插件、移动端 Web 和桌面 Web 都不会把 pending 数量展示成“可换”。

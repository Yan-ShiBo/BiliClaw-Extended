...
📌 最新版本：**v0.3.154（2026-07-05）**

- **Docker 一键部署上线** —— GHCR 预构建多架构镜像，`curl` 一个 compose 文件即可启动，打开 `/setup/` 图形化完成初始化，无需克隆源码。
- **Docker 首启更稳、重建更快** —— 后端不再被 embedding 模型下载卡死；源码重建靠依赖分层缩短到秒级。
- **LLM 探活请求降噪** —— 初始化完成后不再反复发送小额探活请求，账单不再刷屏；探活频率整体降低 10 倍。
- **新增 FAQ 与文档分区** —— 高频安装 / 连接 / 更新问题一页速查，文档导航按用户 / 开发者分区。

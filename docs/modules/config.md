...
局域网 / 远程访问的**可选密码门禁**（`ApiAuthConfig`）。仅当 `enabled=true` 且请求非可信本机时生效；本机（loopback 且无转发头）默认免登录，浏览器插件不受影响。详见 [`docs/modules/api-auth.md`](api-auth.md) 与设计文档 [`docs/archive/plans/2026-05-30-web-password-auth-design.md`](../archive/plans/2026-05-30-web-password-auth-design.md)。
...
| `auto_update_allowed_remotes` | list[str] | BiliClaw Extended GitHub HTTPS / SSH | 允许自动更新快进的 `origin` 精确 allowlist；unknown remote 或带 userinfo/credential 的 URL 都会以 `untrusted_remote` 拒绝 |

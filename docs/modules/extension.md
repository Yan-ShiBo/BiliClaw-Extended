...
| 抖音长批次入队健壮性 | ✅ | `DyTaskQueue.enqueue_with_id()` 对 `database is locked` / `database table is locked` 做 rollback + 短退避重试，避免抖音喜欢分批导入、ChromaDB upsert 和后台写入并发时 `/api/sources/dy/enqueue` 偶发 500；非锁类 SQLite 错误仍按原路径抛出，保证真实数据问题不会被吞掉 |
...
- Chrome Web Store 隐私权政策网址可填写 `https://github.com/Yan-ShiBo/BiliClaw-Extended/blob/main/docs/privacy.md`；该文档说明插件单一用途、权限理由、数据类型、本地后端数据流和无远程代码声明。

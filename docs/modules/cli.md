...
| `fetch-zhihu` | 单独触发知乎事件拉取（不重建画像；浏览 / 收藏夹 / 可选个人动态点赞收藏） | ✅ |
...
用 LLM 整理合并画像里重复的喜欢 / 讨厌主题。兴趣标签和避雷主题会不断积累措辞变体（「智能体开发」vs「智能体开发与实现」），把进入 prompt 的 top-64 名额挤占掉；本命令按「规则合并 → embedding 聚类 → LLM 裁决 → 校验执行」流水线做同义合并，默认整理 likes 权重 top-512 + 全量避雷主题。后台默认每 12 小时自动跑一轮（见 `[scheduler].profile_consolidation_*`），本命令用于手动触发与预览。
...
默认初始化信号上限：B 站观看历史最多 500 条、收藏最多 500 条（跨收藏夹总预算，单个收藏夹会按页补齐）、关注 UP 最多 100 人；小红书 / 抖音 / YouTube 的 `bootstrap_profile` 每个 scope 默认最多 300 条。交互式 `init` 会让用户确认 B 站历史 / 收藏 / 关注上限，历史回车使用 500、收藏回车使用 500、关注回车使用 100；脚本化场景可传 `--bilibili-history-limit N` / `--bilibili-favorite-limit N` / `--bilibili-follow-limit N`。历史传 `0` 表示拉全部可用历史；收藏 / 关注传 `0` 表示跳过对应信号。
...
- `--bilibili-history-limit N` / `--bilibili-favorite-limit N` / `--bilibili-follow-limit N`：覆盖 B 站历史 / 收藏 / 关注初始化信号上限，默认分别为 `500` / `500` / `100`；历史 `0` 表示拉全部可用历史，收藏 / 关注 `0` 表示跳过对应信号。
...
- `OPENBILICLAW_ZHIHU_BOOTSTRAP_MAX_ITEMS` / `OPENBILICLAW_ZHIHU_BOOTSTRAP_MAX_COLLECTIONS`：控制 `fetch-zhihu` 每类最多读取的条目数和最多扫描收藏夹数，默认分别为 `40` / `10`。
...
单独触发知乎 `bootstrap_events` 拉取，用于验证浏览器扩展、知乎登录态和 `/api/sources/zhihu/*` 后端任务桥是否联通。默认采集最近浏览记录与收藏夹条目；传入 `--profile-slug` 后会额外读取该知乎用户主页动态里的点赞 / 收藏动作。

```bash
$ openbiliclaw fetch-zhihu --profile-slug your-slug --wait-seconds 240
知乎 数据拉取
  知乎 浏览 40 条 / 收藏 20 条 / 点赞 12 条
  共抓取并转换 72 条事件；未触发画像生成。
```

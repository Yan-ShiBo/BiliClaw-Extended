...
| 低可用池补货防死锁 | ✅ | `_source_requested_count()` 仍用 raw headroom 限制正常补货规模，但当 `pool_available_count < pool_target_count` 且 raw ceiling 已满时，不再把 source deficit 直接压成 0；raw ceiling 继续由 `_enforce_pool_cap()` / `trim_pool_to_target_count()` 在补货前后修剪，避免大量不可换 raw material 让 Search / producer 永久停摆。 |

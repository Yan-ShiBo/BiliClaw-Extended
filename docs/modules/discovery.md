...
1. **读取画像**
   ...
   真正进入发现策略时，画像会被压缩成更容易消费的结构化摘要。query / rid / domain / keyword planner 生成使用 `build_query_generation_profile_summary()`，只保留稳定的高权重兴趣、兴趣域、核心特质、认知风格、价值观、动机、deep needs、`disliked_topics`、观看风格和探索开放度；flat `interests` 从最多 128 个候选里选出 64 个。如果 embedding cache 已有向量，选择器会用 MMR 风格优先覆盖更多语义簇，并降低贴近 `disliked_topics` 的 interest；`disliked_topics` 自身也做 embedding 多样性去重。没有 cached embedding 时保持原来的权重顺序，不在 prompt 构建热路径新增 embedding 请求。近期觉察、当前洞察、session context、兴趣时间戳和来源 provenance 不进入这类 prompt，避免高频状态把 `discovery.search.queries` / `discovery.trending.rids` / `discovery.explore.queries` / `discovery.keyword_planner` 固定输入撑大。内容评估仍使用自己的 eval profile 压缩口径，保留近期负样本和必要语境。
...
`TrendingStrategy` 并不是把所有分区榜都抓一遍。它会先固定抓 `rid=0` 全站榜，再让 `build_trending_rids_prompt()` 从 compact 画像里挑 3 到 5 个更相关的分区。分区选择关闭额外 core memory 注入和 thinking，输出预算按短 JSON 收口，并按 `profile_kw_digest + 日期` 做约 6 小时进程内缓存；同一天同画像的多轮 trending refresh 会复用 rids，只重新抓榜单内容。
...
2. due 非空 → 现读最新画像 → 取**短事务单飞锁并在调用 LLM 前释放** → `build_merged_keywords_prompt`（一次合并调用、compact 画像只发一份、按平台分块、静态 system 命中 prompt-cache；调用时关闭 thinking 和额外 core memory 注入）→ 解析 `{platform: [...]}` → 按当前 `profile_kw_digest` 写 `pending` 补到 `kw_cache_high`。
...
| 5.2 排行榜策略 | ✅ | 全站榜 + compact 画像选择相关分区榜 + 按画像和日期缓存 rids + LLM 评分筛选 |
...
- 固定拉取 `rid=0` 全站榜
- 再通过 LLM 选择 3 到 5 个相关分区榜；同画像同日的 rids 进程内缓存约 6 小时
- 对每条榜单内容执行 LLM 相关性评估
- 只保留高于阈值的结果

...
| 策略 | 消费的画像字段 | 用途 |
|------|-------------|------|
| **SearchStrategy** | 通过 `build_profile_summary()` 消费 `personality_portrait`, `core_traits[:5]`, `cognitive_style[:5]`, `values[:5]`, `motivational_drivers[:5]`, `current_phase`, `life_stage`, `interest_domains[:8]`, `interests[:64]`, `disliked_topics[:64]`, `deep_needs[:5]`, `style`, `context`, `exploration_openness`, `source_platform_mix`, `mbti`, `recent_awareness[:5]`, `active_insights[:5]`, `_active_speculations[:5]`（兴趣域 / tag 按 weight 排序后截断；`favorite_up_users` 已从画像输出移除） | 生成搜索词、计算兴趣锚定分，并避开长期雷点 |

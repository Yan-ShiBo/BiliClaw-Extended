...
## v0.3.158 / extension v0.3.158: 小红书深扫与跨来源画像权重（2026-07-06）
- **小红书 liked 深扫预算扩大**：扩展的小红书 bootstrap 最大滚动轮次从 30 提高到 160，版本号同步提升到 `0.3.158`，用于覆盖已登录账号里更多可见的喜欢笔记。
- **画像分析按来源与账号做有效权重**：`PreferenceAnalyzer` 会在分析前为事件复制写入 `metadata.analysis_weight`，第二个抖音账号、小红书、B 站信号略微放大；第一个抖音账号按本批新到旧顺序对较旧条目平滑降权，降低第一个抖音上万样本对画像和推荐分析的挤占。
- **权重进入 prompt 与来源分布**：`analysis_weight`、`source_account_id/account_id`、`source_weight_multiplier`、`recency_weight_multiplier` 进入 prompt compact 白名单；`source_platform_mix` 在有 `analysis_weight` 时改按有效权重归一化，没有该字段的旧调用保持原始计数兼容。
- **验证**：`pytest tests/test_preference_analyzer.py -q` 通过。
...
- **已在目标 tab 时不再重新点击导航**：content script 会识别当前是否已经在 `/user/...?...showTab=like` 等目标 profile tab；已经在目标页时直接继续滚动和采集，减少“重新点喜欢后回到顶部”的情况。
- **task-result 确认不再等待画像 LLM**：抖音喜欢 partial 回传现在会先完成事件/向量入库和 HTTP ack，再把增量画像更新放到后台 best-effort 执行；当本机或远端 Ollama 临时不可用时，扩展不会因为画像分析连接失败而卡住下一次 final ok。
- **边界说明**：浏览器关闭、扩展重载、用户手动关闭该抖音页签，或 Douyin 自己刷新/虚拟列表回收时，仍可能失去页面滚动上下文；这种情况下会自动退回新开页签并依靠已见 key 跳过旧条目。
- **测试与版本号**：新增页签复用 URL 判定和 profile scope route 判定测试；扩展 manifest 与 npm 元数据提升到 `0.3.155`，便于手动重载后确认新版本。

## v0.3.154 / extension v0.3.154: 抖音多账号分账续跑与扩展版本号可见（2026-07-05）

- **抖音多账号分开处理**：`bootstrap_profile` 任务现在携带 `account_id`，第一个账号使用 `primary`，切换到第二个账号后使用 `account2`；两个账号的 `dy_seen_video_keys`、`dy_scope_progress`、画像事件和向量库文档互不覆盖。
- **抖音采集页不再超时自动消失**：前台导入任务超时时保留抖音页面，便于观察和继续排查；liked 导入的超时预算会根据滚动轮次和已跳过条目动态放宽。
- **抖音续跑真正按已见 key 跳过**：修复 content script 没把 `skip_item_keys` 传入 `BootstrapItemSink` 的遗漏；现在浏览器端会跳过已处理喜欢，已处理条目不再占用 `max_new_items_per_scope` 本批新增名额。
- **扩展版本号同步提升**：Chrome manifest、扩展 npm 元数据和项目版本统一提升到 `0.3.154`，重新加载扩展后可在浏览器扩展页确认新版本。

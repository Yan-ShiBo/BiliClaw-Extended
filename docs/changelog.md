...
- **Docker 安装现代化**：新增 `.github/workflows/release-docker.yml`，`backend-v*` tag 自动构建并推送多架构（amd64 + arm64）镜像到 GHCR；新增自包含的 `docker-compose.prebuilt.yml`，用户 `curl` 一个文件即可 `docker compose up -d`，无需克隆源码或本地构建。Dockerfile 改为依赖分层（先按 `pyproject.toml` 装依赖再拷源码 `pip install --no-deps .`），源码变更后重建从整装依赖缩短到十几秒。`docker-compose.yml` 后端对 Ollama sidecar 的依赖从 `service_healthy` 放宽为 `service_started`，bge-m3 首拉失败或缓慢不再永久卡死后端，`/setup/` 前置检查会显示 embedding 就绪状态。`docs/docker-deployment.md` 重写快速开始（预构建镜像 / 源码构建 / 终端向导三路径，主路径统一收口到 `/setup/` 图形化引导）、修正容器内不存在的 `uv run` 命令、修正安装向导来源覆盖描述（bootstrap 终端向导只含 B 站 / 小红书 / 抖音 / YouTube opt-in，X / 知乎 / Reddit 在 `/setup/` 或设置页开启）。本机真实验证：镜像构建、改源码重建 17s（依赖层 CACHED）、容器 8s 内 `/api/health` 200、`/setup/` 200、ollama 种子配置与 `rdt` / `twitter_cli` 内置依赖齐全。GHCR 首个镜像（`0.3.152` + `latest`）已由 workflow_dispatch 发布且匿名可拉取；完整用户视角 E2E 通过：`curl` compose 文件 → `docker compose up -d` → 后端秒级健康、不被模型下载阻塞 → sidecar 后台拉取 bge-m3 → `embedding_ready` 约 140s 自动就绪。官网首页安装区新增「Docker 预构建镜像」双语面板（带复制按钮），`docs/faq.md` 新增 Docker 部署 / 升级条目，Docker 文档 FAQ 补充新旧 compose 同名容器冲突的处理方式。聚合 Release 页新增 Docker 渠道行：`sync-aggregate-release.sh` 匿名探测 GHCR 上该版本 manifest 是否可拉取，可拉取才展示镜像引用与 compose 下载指引（与其他 channel 同样不回填）；`release-docker.yml` 推完镜像后在 tag push 场景自动重跑聚合页同步，避免镜像晚于其他渠道完成导致漏显示；顺带修正 Firefox XPI 缺失时 "use No signed XPI…" 的病句文案。已用 notes-only 方式实测刷新线上 `openbiliclaw-v0.3.152` 聚合页。
